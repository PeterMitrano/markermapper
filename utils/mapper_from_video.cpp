...
int main(int argc,char **argv){
    try{
        ...
        if (argc<4 || cml["-h"]){
            cerr<<"Usage: (live | intput-video.avi[:v2.avi[:v3.avi...]]) camera_parameters.yml marker_size(meters)   [-ref reference_marker_id] [-fi frameIncrement] [-out basename] [-d maker_dictionary]"<<endl;
...
//        AMM->saveToFile(outBaseName+".amm");
        ...
        Viewer.setParams(0.5,1280,960,"map_viewer");
        key=0;
        while(key!=27)
            key =     Viewer.show(mmap );
